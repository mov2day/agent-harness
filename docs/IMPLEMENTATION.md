...
| 6 | Authenticated React controls, alerts, durable recovery, conformance and release tooling | Controls and API implemented; browser sign-in verified; 87 tests and builds pass on macOS/Linux; measured authorization target passes; full conformance/release tooling and QA pending |
...
Latest verification: 2026-09-23, **87/87 tests**, type checks and production builds passed on macOS arm64 and Linux arm64. The Linux image was `49f4107d4f7f`. The real OpenCode transport, engine integration and complete specialist workflow tests passed on the macOS host using runtime image `411216a2fdc7`. See [conformance evidence](../conformance/README.md) for scope and benchmark limitations. The overall goal remains incomplete; these results do not establish full release conformance.
