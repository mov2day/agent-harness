...
const pattern = z
  ...
  .refine(
    (p) =>
      ...
      !p.includes("\0") &&
      !p.startsWith("!"),
    "Expected a relative path pattern",
  );
export const policySchema = z
  .object({
    version: z.literal(1),
    allowPaths: z.array(pattern),
    denyPaths: z.array(pattern),
    tools: z.array(
      z.enum([
        "read",
        "change",
    ...
    deletion: z.boolean(),
    maxSpecialists: z.number().int().min(0).max(32),
    ...
    models: z.record(
      z.enum(roles),
      z.object({ model: z.string(), reasoning: z.string() }).strict(),
    ),
  })
  .strict();
...
export class Policies {
  ...
  private assertTightening(g: Policy, p: Policy) {
    ...
    for (const f of ceilings)
      if (p[f] > g[f]) errors[f] = "Cannot raise the global ceiling";
    if (p.deletion && !g.deletion) errors.deletion = "Globally forbidden";
    for (const field of ["tools", "domains"] as const)
      if (p[field].some((x) => !(g[field] as string[]).includes(x)))
        errors[field] = "Cannot enable globally forbidden resources";
    if (
      p.commands.some((c) => !g.commands.some((gc) => digest(gc) === digest(c)))
    )
      errors.commands =
        "Command must be globally approved with identical constraints";
    if (g.humanGates.some((s) => !p.humanGates.includes(s)))
      errors.humanGates = "Cannot remove a global human gate";
    ...
    if (
      p.allowPaths.some(
        (x) =>
          !g.allowPaths.includes(x) &&
          (/[*!?{[\]]/.test(x) || !g.allowPaths.some((gp) => matches(x, gp))),
      )
    )
      errors.allowPaths = "Use a global pattern or a permitted concrete path";
    if (Object.keys(errors).length)
      throw new HarnessError(
        "policy_escalation",
        "Repository policy exceeds global authority",
        400,
        errors,
      );
  }
  active(scope: string): PolicyRecord {
    ...
    const record = this.store.get<PolicyRecord>(
      "policy",
      `${scope}:${pointer.id}`,
    );
    check(
      record &&
        policySchema.safeParse(record.policy).success &&
        digest(record.policy) === record.id,
      "policy_unavailable",
      "Active policy is invalid",
    );
    return record;
  }
  effective(repository: string): Effective {
    ...
    return {
      ...
      id: digest([global.id, local?.id ?? null]),
      global,
      repository: local,
      policy,
      rules: [
        `global:${global.id}`,
        ...(local ? [`repository:${local.id}`] : []),
      ],
    };
  }
  path(e: Effective, path: string): boolean {
    ...
    const nocase =
      this.store.get<{ caseSensitive: boolean }>("repository", e.repositoryId)
        ?.caseSensitive === false;
    return [e.global, ...(e.repository ? [e.repository] : [])].every(
      (r) =>
        r.policy.allowPaths.some((p) => matches(path, p, nocase)) &&
...
export const matches = (path: string, glob: string, nocase = false) =>
  minimatch(path, glob, {
    dot: true,
    nocase,
    nonegate: true,
