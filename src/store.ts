...
export class Store {
  ...
  constructor(path: string, clock?: Clock) {
    ...
    this.db.exec(`PRAGMA journal_mode=WAL; PRAGMA busy_timeout=5000; PRAGMA synchronous=FULL; PRAGMA foreign_keys=ON;
    ...
    `);
    this.clock = clock ?? new EngineClock(this.get<{time:number}>('meta','clock')?.time);
  }
  get<T>(kind: string, key: string): T | undefined {
    const row = this.db.prepare('SELECT data FROM records WHERE kind=? AND id=?').get(kind,key);
    return row ? JSON.parse(row.data as string) as T : undefined;
  }
  list<T>(kind: string, repository?: string, session?: string): T[] {
    const sql = 'SELECT data FROM records WHERE kind=?' + (repository === undefined ? '' : ' AND repository=?') + (session === undefined ? '' : ' AND session=?');
    const args = [kind, ...(repository === undefined ? [] : [repository]), ...(session === undefined ? [] : [session])];
    return this.db.prepare(sql).all(...args).map(row => JSON.parse(row.data as string) as T);
  }
  put(kind: string, key: string, value: unknown, repository = '', session = '') {
    this.db.prepare('INSERT INTO records(kind,id,repository,session,data) VALUES(?,?,?,?,?) ON CONFLICT(kind,id) DO UPDATE SET repository=excluded.repository,session=excluded.session,data=excluded.data').run(kind,key,repository,session,JSON.stringify(value));
  }
  ...
  transaction<T>(fn: () => T): T {
    ...
    try {
      const result = fn();
      check(!(result instanceof Promise), 'async_transaction', 'Transactions cannot contain asynchronous effects');
  ...
  audit(event: string, data: unknown, repository = '', session = '') {
    this.db.prepare('INSERT INTO audit(id,time,repository,session,event,data) VALUES(?,?,?,?,?,?)').run(id(),this.clock.now(),repository,session,event,JSON.stringify(data));
  }
  close() { this.db.close(); }
}
