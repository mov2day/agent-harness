...
export class Gateway {
  ...
  async fetch(op: Operation, signal: AbortSignal): Promise<Evidence> {
    ...
    if (cached) {
      check(
        cached.repository === op.repository && cached.session === op.session,
        "cache_scope",
      );
      return cached;
    }
    const chain: string[] = [];
    let target = op.args.url;
    for (let redirects = 0; redirects <= 5; redirects++) {
      const e = this.operations.validate(op),
        policy = this.operations.policies.effective(e.repository).policy;
      const url = researchUrl(target, policy.domains);
      const records = await this.resolve(url.hostname, {
        all: true,
        verbatim: true,
      });
      check(
        records.length > 0 && records.every((r) => publicAddress(r.address)),
        "prohibited_address",
      );
      ...
      if ([301, 302, 303, 307, 308].includes(response.status)) {
        check(redirects < 5 && response.location, "redirect_limit");
      ...
      check(
        ...
        `Research server returned ${response.status}`,
      );
      this.operations.validate(op);
  ...
  private connect(
    ...
    return new Promise((resolve, reject) => {
      ...
      const req = this.request(
        ...
        (res) => {
          ...
          res.on("data", (chunk: Buffer) => {
            bytes += chunk.length;
            if (bytes > 256 * 1024) {
              res.destroy(new Error("research_result_limit"));
              return;
            }
            chunks.push(chunk);
          });
      ...
      req.setTimeout(10_000, () => req.destroy(new Error("research_timeout")));
      req.on("error", reject);
      req.end();
    });
  }
