...
export class CommandWorkers {
  ...
  async execute(op: Operation, signal: AbortSignal): Promise<WorkerResult> {
    ...
    const args = [
      ...
      "--workdir",
      `/workspace/${op.args.cwd}`,
    ];
    for (const [key, value] of Object.entries(op.args.env)) {
      check(
    ...
    const cancel = async () => {
      killRequested = true;
      try {
        await exec("docker", ["kill", name], { timeout: 5000 });
        return true;
      } catch {
        return false;
      }
    ...
    return new Promise((resolve, reject) => {
      ...
      const append = (data: Buffer) => {
        const remaining = 256 * 1024 - Buffer.byteLength(output);
        if (data.length > remaining) truncated = true;
        if (remaining > 0)
          output += data.subarray(0, remaining).toString("utf8");
      };
      ...
      child.on("close", (code) => {
        ...
        if (killRequested) reject(new Error("worker_cancelled"));
        else
          resolve({ exitCode: code ?? -1, output, truncated, container: name });
      });
    });
  }
}
