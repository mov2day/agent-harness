...
export function openCodeHooks(relay: RuntimeRelay, sessionID: string) {
  return {
    "tool.execute.before": async (
      ...
      check(
        harnessTools.has(input.tool),
        "native_tool_denied",
        `Tool ${input.tool} must use the engine broker`,
      );
    },
    "experimental.session.compacting": async (
      ...
      check(input.sessionID === sessionID, "runtime_session_spoof");
      const context = await relay.context(sessionID);
      output.context.push(JSON.stringify({ authoritativeCheckpoint: context }));
    },
    dispatch: async (tool: string, args: unknown, callID: string) => {
      check(harnessTools.has(tool), "native_tool_denied");
      return relay.execute({
        session: sessionID,
        tool: tool.slice("harness_".length),
        args,
        call: callID,
      });
    },
...
export const openCodeRuntimeProfile = {
  runtime: "opencode",
  protocol: "plugin-1.x",
  requiredHooks: ["tool.execute.before", "experimental.session.compacting"],
  nativeTools: "deny",
